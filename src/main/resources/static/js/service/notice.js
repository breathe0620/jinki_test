...
$('#detailBtnUpdate').on('click', function () {
    ...
    var options = {
        'type': 'GET',
        'url': '/service/notice/' + $('#detailNoticeId').text() + '/' + langType,
...
// 상세보기 이동
$('tr[name=list] td:not([name=noticeId])').locationListView('[name=noticeId]', '/service/notice/');

// 등록 버튼 클릭시
$('#btnCreate').on('click', function () {
    window.sessionStorage.setItem('url', window.location.href);
    window.location.href = '/service/notice';
});
...
$('#btnDelete').on('click',function(){
    ...
    var options = {
        'url'         : '/service/notice/'+ noticeId,
        'type'        : 'DELETE',
        'callback' : function acallback(result){
            window.location.href = '/service/notices';
        }
    };
...
$('#createBtnSave').on('click', function () {
    ...
    if ($('#formNotice').parsley().validate()) {
        var options = {
            'type': 'POST',
            'url': '/service/notice',
...
$('#updateBtnSave').on('click', function () {
    ...
    param.noticeNo    = $('#detailNoticeId').text();
    ...
    if ($('#formNotice').parsley().validate()) {
        var options = {
            'type': 'PUT',
            'url': '/service/notice/' + param.noticeNo,
            'data': JSON.stringify(param),
            'contentType': 'application/json; charset=utf-8',
            'dataType': 'json',
            'callback': function acallback(result) {
